...
class LifeGoalAnalyticsService:
    ...
    async def get_life_goals_summary(self, user_id: UUID) -> Dict:
        ...
        active_goals = len([g for g in life_goals if g.status == "active" or g.status == "in_progress"])
        ...
        return {
            ...
            "avg_days_to_complete": round(avg_days_to_complete, 1) if avg_days_to_complete else None
        }

    async def get_milestone_statistics(self, user_id: UUID, goal_id: UUID = None) -> Dict:
