...
class HabitService:
    ...
    async def create_entry(
        ...
        existing = await self.entry_repository.get_entry_by_date(habit_id, entry_data.entry_date)
        if existing:
            raise HTTPException(
...
class WorkoutService:
    ...
    async def create_workout(self, user_id: UUID, workout_data: WorkoutCreate) -> Workout:
        """Create a workout with exercises."""
        workout = Workout(
            user_id=user_id,
            **workout_data.model_dump(exclude={"exercises"})
        )
...
class DailyReviewService:
    ...
    async def create_review(self, user_id: UUID, review_data: DailyReviewCreate) -> DailyReview:
        """Create a daily review."""
        # Check if review already exists for this date
        existing = await self.repository.get_review_by_date(user_id, review_data.review_date)
        if existing:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
class BlogEntryService:
    ...
    async def create_blog_entry(self, user_id: UUID, entry_data: BlogEntryCreate) -> BlogEntry:
        """Create a blog entry."""
...
class ProgressSnapshotService:
    ...
    async def create_snapshot(self, user_id: UUID, snapshot_data: ProgressSnapshotCreate) -> ProgressSnapshot:
