...
class HabitRepository(BaseRepository[Habit]):
    ...
    async def count_user_habits(self, user_id: UUID, is_active: Optional[bool] = None) -> int:
...
class HabitEntryRepository(BaseRepository[HabitEntry]):
    ...
    async def get_entry_by_date(self, habit_id: UUID, entry_date: date) -> Optional[HabitEntry]:
...
class FoodRepository(BaseRepository[Food]):
    ...
    async def get_user_foods(
        ...
        query = query.offset(skip).limit(limit).order_by(Food.meal_date.desc(), Food.meal_time.desc())
...
class DailyReviewRepository(BaseRepository[DailyReview]):
    ...
    async def get_review_by_date(self, user_id: UUID, review_date: date) -> Optional[DailyReview]:
...
class BlogEntryRepository(BaseRepository[BlogEntry]):
    ...
    async def count_user_blog_entries(self, user_id: UUID, status: Optional[str] = None) -> int:
...
class ProgressSnapshotRepository(BaseRepository[ProgressSnapshot]):
    ...
    async def get_user_snapshots(
        ...
        query = query.offset(skip).limit(limit).order_by(ProgressSnapshot.snapshot_date.desc())
